if (Meteor.isDevelopment) {
    if (Meteor.settings.private?.SENDER_EMAILS ) {
        ...
        console.warn('[Meteor + Vue] - Sender emails are not configured. Emails will not be sent');
    }
}


const nameT = 'Scaffold Meteor + Vue';
const email = `<${process.env.EMAIL_SERVICES}>`;

const from = `${nameT} ${email}`;

Accounts.emailTemplates.siteName = nameT;
Accounts.emailTemplates.from = email;

const productSrc='https://firebasestorage.googleapis.com/v0/b/meteor-vue-3cafc.appspot.com/o/vue-meteor.png?alt=media';
const logoSrc='https://firebasestorage.googleapis.com/v0/b/meteor-vue-3cafc.appspot.com/o/vue-meteor.png?alt=media';
...
if (Meteor.isDevelopment) {
    if (Meteor.settings.private?.MAIL_URL ) {
        ...
        console.warn('[Meteor + Vue] - Email settings are not configured. Emails will not be sent');
    }
}